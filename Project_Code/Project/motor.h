...
void drive_verichel (char direct, char delay_time =0 )  //1800 around 1 cc 
{
  ...
  {
    case front : 
        motore1.writeMicroseconds(1250); //left foreward
        motore2.writeMicroseconds(1250);  //right backward 
        if (delay_time !=0 )
        {
    ...
    case left : 
        motore1.writeMicroseconds(1250); //left foreward
        motore2.writeMicroseconds(1750);  //right backward 
         if (delay_time !=0 )
        {
        delay(100*delay_time); // distance
        stop_function();
        ...
        break ;
    case right : 
        motore1.writeMicroseconds(1750); //left foreward
        motore2.writeMicroseconds(1250); //right backward 
         if (delay_time !=0 )
        {
        delay(100*delay_time); // distance
